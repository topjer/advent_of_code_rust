...
struct Game {
    ...
    direction: String,
    loop_points: HashSet<(usize, usize, String)>
}
...
impl Game {
    ...
    fn make_move(&mut self) {
        ...
        self.loop_points.insert((self.position.0, self.position.1, self.direction.clone()));
        if current_field == "|" {
...
fn logic_part_2 (input: &Vec<String>) -> u32 {
    ...
    for row_index in 0..game.map_dimensions.0 {
        ...
        for column_index in 0..game.map_dimensions.1 {
            if game.loop_points.contains(&(row_index, column_index, "u".to_string())) {
                if game.get_field((row_index, column_index)) == "|" {
                    inside = !inside;
                } else {
                    inside = false;
                }
